...
|---|---|---|
| maintainer | Final authority, risk acceptance, Git/release executor | `agents/organization/AUTHORITY.md:3-14` |
| Claude and Codex | Technical actors below maintainer authority | `AGENTS.md:9-16` |
...
|---|---|---|---|---|---|---|
| Current repository | Governance instructions | Root `AGENTS.md` is canonical; Claude imports it | Tracked repository files | maintainer, Claude, Codex, delegated workers | Review plus maintainer-only governance gate | `AGENTS.md:42-48`; runtime parser behavior remains client-dependent |
| Current repository | Git mutation | User mandate and RBAC deny all agents | maintainer-only local Git/GitHub action | maintainer | Human authority; agents must refuse | `AGENTS.md:18-22`; `agents/rbac/permissions.yaml:46-52` |
| Planned local MCP | Ruflo server process | Non-executable example; disabled by default | Local stdio entrypoint, no network listener | Approved local clients only | maintainer activation gate and client sandbox | `config/ruflo-mcp.example.toml:1-11`; no approved runtime exists |
...
| Planned worker test | One Codex child process | Exact bundled adapter path and 60-second command timeout | Read-only fixture in `orchestration/ruflo/fixtures/` | Orchestrator and one Codex worker | Explicit maintainer authorization, bounded prompt, exit/orphan checks | `scripts/ruflo-dual-readonly-smoke.sh`; not executed |
| Isolated dependency audit | Ruflo package graph | Exact `ruflo@3.38.21`, scripts disabled | Temporary directory outside repository | the reviewer audit process | Temporary isolation and cleanup | `orchestration/ruflo/SECURITY_BOUNDARY.md:19-29` |

## 2. Threat Model, Trust Boundaries, and Assumptions

### Protected assets and objectives

- maintainer authority and the integrity of approvals, RBAC, governance, decisions, and task scope.
...
- **maintainer:** trusted final authority; may intentionally approve consequential action. Compromise of the maintainer account or host is outside what repository policy alone can prevent.
...
- **Malicious contributor or untrusted content author:** may control repository text, issue content, prompts, dependency metadata, or memory values, but does not start with maintainer approval or host credentials.
...
1. **maintainer → governance:** human direction becomes durable policy. Only explicit maintainer decisions cross this boundary; silence and tool output are not approval (`AGENTS.md:24-30`).
...
7. **Agent → Git/GitHub:** this boundary is closed to agents. Only the maintainer may execute mutations or releases (`agents/organization/AUTHORITY.md:12-18`).

### Assumptions and open questions

- The host, maintainer account, and installed Claude/Codex clients are not already compromised.
...
| High | Agent or orchestrator performs a prohibited Git mutation or release | Tool access plus failure to enforce repository policy | Unauthorized history, branch, remote, or release changes | Explicit absolute prohibition in governance and RBAC | Remove mutation tools from agent environments; maintainer performs actions manually after review | `AGENTS.md:18-22`; `agents/rbac/permissions.yaml:46-73` |
...
- **Critical:** unauthenticated or low-privilege code execution with the Ruflo process's meaningful host authority, or compromise that directly yields maintainer-equivalent release/secret authority. A package advisory is not automatically Critical for NOETARCH if its parser is unreachable and the process is strongly sandboxed.
